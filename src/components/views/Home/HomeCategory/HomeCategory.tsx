      ...
      <CardBody className="mt-4 p-0">
        <div className='grid auto-cols-[8rem] grid-flow-col gap-4 overflow-x-auto lg:grid-cols-8'>
          {!isLoading ? categories?.map((category) => (
            <Link key={`category-${category?._id}`} href={`/event?category=${category._id}`} className='flex aspect-square cursor-pointer flex-col items-center justify-center gap-2 rounded-xl p-4'>
            ...
            Array.from({ length: 8 }).map((_, index) => (
              <Skeleton key={`category-skeleton${index}`} className='aspect-square rounded-xl' />
            ))
          )}
        </div>
      </CardBody>
